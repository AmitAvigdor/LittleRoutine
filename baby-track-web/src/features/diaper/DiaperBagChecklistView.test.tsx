...
describe('DiaperBagChecklistView', () => {
  ...
  it('shows empty, incomplete, and complete status messages based on packed quantities', async () => {
  ...
  it('lets users add and edit a custom item from the sticky action', async () => {
    ...
    expect(screen.getByText('Custom item • Tap to edit or remove')).toBeInTheDocument();
    ...
    expect(localStorage.getItem(STORAGE_KEY)).not.toContain('targetQuantity');
  });
